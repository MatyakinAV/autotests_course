...
def repeats(our_str):
    ...
    for k in our_str:
        if letters_dict.get(k) == None:  # если такого значения нет
            ...
            q = letters_dict.get(k) + 1  # записываем кол-во повторений буквы + 1
            letters_dict.update({k: q})  # записываем в словарь новое значение к ключу
            new_str = new_str + f'{k}_{q}'  # добавляем в строку значение и кол-во повторений
    return new_str
