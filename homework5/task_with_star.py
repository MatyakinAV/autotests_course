...
def to_roman(val):
    slovar = {"M": 1000, "CM": 900, "D": 500, "CD": 400, "C": 100, "XC": 90, "L": 50, "XL": 40, "X": 10, "IX": 9,
              "V": 5, "IV": 4, "I": 1}
    roman_str = ''
    while val > 0:  # пока число больше нуля
        for j in slovar.keys():  # начинаем идти по словарю
            while val - slovar.get(j) >= 0:  # пока число больше либо равно нулю при вычитании значения по ключу
                val -= slovar.get(j)  # вычитаем
