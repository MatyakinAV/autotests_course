...
def everything_for_your_cat(cats_data):
    slovar = {}  # словаре для сборки владельца и его котов
    our_str = ''
    for p in cats_data:  # берем каждый кортеж из списка
        cats = p[0] + ', ' + str(p[1])  # берем имя котика и его возраст
        slovar.setdefault(p[2:], []).append(
            cats)  # проверяем есть ли в нашем словаре такой владелец, если нет добавляем
        # владельца и ему котика, если есть владелец,то только котика
    for k, v in slovar.items():  # берем  ключ значение по каждому элементу
        our_str = our_str + ' '.join(k) + ': ' + "; ".join(v) + '\n'  # собираем строчку из имя - коты
    return our_str
