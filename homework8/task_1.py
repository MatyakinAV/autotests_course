...
def treatment_sum(our_tuple):
    """
    На вход кортеж, если 2 элемента и их можно сложить возвращаем результат, если нельзя сложить - кидаем исключения
    :param our_tuple:
    :return: сумму значений кортежа или исключения
    """
    try:
        nums = len(our_tuple)
        if nums < 2:
